...
+echo "-=-=-=- Adding Gridcoin Neural Netowrk Nodes - Updated 03/06/17 -=-=-=-"
...
+echo "-=-=-=- Finished adding current network list -=-=-=-"
+echo "-=-=-=- Adding GeoIP Round Robin DNS -=-=-=-"
+echo "addnode=gridcoin.is.dopeshit.net" | sudo tee -a $config
+echo "-=-=-=- This makes sure as a failsafe you are connected to the closest node to you -=-=-=-"
